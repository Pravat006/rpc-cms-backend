...
import { GlobalSettings } from './app/modules/global-settings/global-settings.model';
import { Page } from './app/modules/page/page.model';
import { Faculty } from './app/modules/faculty/faculty.model';
import { TeamMember } from './app/modules/team/team.model';
import { Post } from './app/modules/post/post.model';
import { GalleryImage } from './app/modules/gallery/gallery.model';
...
export const seedDefaultAdmin = async () => {
    try {
        const adminEmail = config.DEFAULT_ADMIN_EMAIL;
...
const seedGlobalSettings = async () => {
    ...
    logger.info("[SEED] Global Settings seeded");
};
            ...
            },
            {
                ...
                data: {
                    ...
                    amenities: [
                        ...
                        { icon: "utensils", name: "Hygienic Mess" },
                        { icon: "bed", name: "Comfortable Rooms" },
                        { icon: "library", name: "Study Rooms" },
                        { icon: "medical", name: "First Aid" },
                        { icon: "coffee", name: "Recreation Area" },
                        { icon: "transport", name: "Transport Facility" },
                        { icon: "tv", name: "Common TV Room" },
                        { icon: "book", name: "Library Access" },
                        { icon: "home", name: "Homely Environment" },
                        { icon: "user", name: "Warden Supervision" }
                    ]
                }
            },
            {
...
const seedFaculty = async () => {
    ...
    logger.info("[SEED] Faculty seeded");
};
...
const seedGallery = async () => {
    ...
    await GalleryImage.create(images.map((url, index) => ({
        url,
        alt: `Gallery Image ${index + 1}`,
        album: "General",
        order: index
    })));
    logger.info("[SEED] Gallery seeded");
};
...
const seedFacultyPage = async () => {
    await Page.deleteMany({ slug: "faculties" });
    await Page.create({
        title: "Faculties",
        slug: "faculties",
        backgroundImage: UNSPLASH_IMAGES.backgrounds.campus,
    ...
    logger.info("[SEED] Faculty page seeded");
};
...
import { Navigation } from "./app/modules/navigation/navigation.model";
...
const seedNavigation = async () => {
    ...
    await Navigation.create({ menu: linkPages(menu) });
    logger.info("[SEED] Navigation seeded");
};
